...
int* matchingStrings(int strings_count, char** strings, int queries_count, char** queries, int* result_count) {
        int* count = (int*)malloc(queries_count * sizeof(int));

    for (int qIndex = 0; qIndex < queries_count; qIndex++) {
        int matchTotal = 0;

        for (int sIndex = 0; sIndex < strings_count; sIndex++) {
            char* queryWord = queries[qIndex];
            char* currentWord = strings[sIndex];

            int same = 1;  
            int k = 0;
            while (queryWord[k] != '\0' || currentWord[k] != '\0') {
                if (queryWord[k] != currentWord[k]) {
                    same = 0;
                    break;
                }
                k++;
